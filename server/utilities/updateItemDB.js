var siteBaseURL ="http://mabinogi.x10.mx/itemdb/?mode=comparenew&rev1=184&rev2=191&show=100&hide1=0&hide2=0&page="
...
var commitItem = function(item){
	return fs.appendFileSync('./itemDbUpdate.json', JSON.stringify(item));
}
var pageCount = process.argv[2];
var url = (process.argv.length > 3) ? process.argv[3] : siteBaseURL; 
fs.appendFileSync('./itemDbUpdate.json', '[');
for(var x = 0; x < pageCount; x++){
	getTable(x, function(res){
		fs.appendFileSync('./itemDbUpdate.json', JSON.stringify(res, undefined, 2) + ',\n');
	});
}
