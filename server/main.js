...
{ //Erinn Exchange Data Layer functions
	...
	var ee_getNewestListings = function (dat, callback) {
		try {
			ee_ListingModel.find({'open': true}).sort({
				'timestamp': -1
	...
	var ee_getItemByName = function (name, callback) {
		ee_ItemModel.find({
			...
			if(err){ return callback(err, false);}
			if(res.length <=0){ callback(null, false);}
			...
			ee_itemListingIsValid({
				...
				if (ok === true) {
					...
					return toAdd.save(function (err) {
						if (err) return callback(err, false);
						return 	ee_UserModel.find({'user': dat['user']}).limit(1).exec(function(err, res){
							if(err) return callback(err, false);
	...
	var ee_getHighestBid = function(listing){
		if(listing.bids.length <= 0){
			return { user : 'none', amount : listing.price};
		}
				...
				if (ee_checkBidBounds(result[0], highestBid , data['bid']) === false) {
					...
					return newBid.save(function(err){
						...
						return result[0].save(function (err) {
							if (err) return callback(err, false);
							return ee_UserModel.find({'user' : data['user']}).limit(1).exec(function(err, res){
								if(err) return callback(err, false);
		...
		try {
			...
			if(['pricegt', 'pricelt', 'priceeq'].indexOf(type) >= 0){
				...
				if(data['exact'] == true){
					...
					opt[type] = { $regex : new RegExp(escape(data[type]), 'i')};
				}
			} else if(type === 'empty') {
			...
			opt['price'] = {'$gt' : Number(data['priceMin']) , '$lt' : Number(data['priceMax'])}  ;
			opt['server'] = util.isArray(data['server[]']) ? { '$in' : data['server[]'] } : data['server[]'];
	...
	var ee_getAccountData = function(user, callback){
		return ee_UserModel.find({'user' : user}).populate('bids listings').limit(1).exec(function(err, res){
			if(err) return callback(err, false);
			return callback(null, res[0]);
		});
	};
	...
	var ee_accountAddCharacter = function(dat, callback){
		return ee_UserModel.find({'user': dat['user']}).exec(function(err, res){
			if(err){
				return callback(err, false);
			}
			...
			res[0].characters.push({'name' : dat['characterName'], 'server' : dat['server']});
			return res[0].save(function(err){
				if(err){
					return callback(err, false);
				}
	...
	var ee_closeListing = function(dat, callback){
		return ee_ListingModel.find({'listingID' : dat['listingID']}).exec(function(err, listing){
			if(listing.length <= 0){
	...
	var ee_editListing = function(dat, callback){
		return ee_ListingModel.findOne({'listingID': dat['listingID']}).exec(function(err, res){
	...
	var ee_getUserBidsWithListings = function(dat, callback){
		ee_BidModel.find({'user' : dat['user']}).populate('listingRef').exec(function(err, res){
...
{ //Erinn Exchange Routes / Express Application
	...
	ee_app.use(function(req, res, next){
		fs.appendFile('./access-log.txt', sprintf("[%s]IP: %s, LoginSession: %s\n",(new Date(Date.now())).toString(), req.connection.remoteAddress, JSON.stringify(req.LoginSession)), function(err){
			if(err) { console.log("Access Log: " + err); return next(); }
			return next();		
		});
	})
	ee_app.use(function(req, res ,next){
		if(req.path.indexOf('/api/admin') > -1){
			fs.appendFile('./admin-log.txt', sprintf("[%s]IP: %s, Authenticated User: %s, URL: %s, QueryString: %s", new Date(Date.now()), req.connection.remoteAddress, req.LoginSession != {} ? req.LoginSession.user : "None", req.path, req.params.toString()), function(err){
				if(err) { console.log("Admin Log: " + err); return next(); }
				return next();
			});
		} else {
			return next();
	...
	var ee_routes = function (ee_app) {
			...
			if (req.LoginSession.authenticated) {
				...
				if (containsKeys(dat, ['itemName', 'itemID', 'server', 'tags[]', 'price', 'auction', 'publicAfterClose', 'timeout'])) {
					if(dat['price'] > 50000000){
						return res.end(JSON.stringify({'success': false, 'error': "Price is too damn high!"}));
					}
					...
					ee_listingAddNew(dat, function (err, result) { 
						...
						console.log(err);
						if (err) res.end(JSON.stringify({success : false , error : err.toString()}));
						if (result) {
							ee_ListingModel.find({
								...
								res.write(JSON.stringify({success: true, result : 'Listing Submitted!', listingID : ret}));
								return res.end();
							});
						} else {
							return res.end(JSON.stringify({success : false, error :'failed to create listing!'}));
						}
					});
				} else {
					res.end(JSON.stringify({success: false, error:'err: missing parameters!'}));
				}
			} else {
			...
			if (containsKeys(dat, ['auction', 'type[]'])) {
				...
				if (dat['type[]'][0] == 0) {
					...
					if (dat['type[]'][1] ==='tags' && dat['tags[]'] != undefined){
						...
						return res.end(JSON.stringify({success: false, error: 'No type specified, or type value not defined!'}));
					}
				} else {
					//complex search
		...
		ee_app.get('/api/bid', function (req, res) {
			...
			if (containsKeys(dat, ['listingID', 'bid'])) {
				if (req.LoginSession.authenticated === true) {
					if(dat['bid'] > 55000000){
						return res.end(JSON.stringify({'success': false, error : 'Bid amount beyond max ceiling for listings.'}));
					}
					dat['user'] = req.LoginSession.user;
					ee_makeBid(dat, function (err, result) {
						if(err){
							...
							res.end(JSON.stringify({success : true, result : false}));
						}
					});
				} else {
					res.end(JSON.stringify({ success : false, error : 'err: Not Logged In.'}));
				}
			} else {
				res.end(JSON.stringify({success : false, error : 'err: missing parameters!'}));
			}
		...
		ee_app.get('/api/getBidsByUser', function(req, res){
			...
			if(containsKeys(dat, ['user'])){
				ee_getUserBidsWithListings(dat, function(err, res){
					if(err){ return res.end(JSON.stringify({'success' : false, 'error' : err})); }
					return res.end(JSON.stringify({'success' : true, 'result' : res.toString()}));
				});
			} else {
				return res.end(JSON.stringify({'success' : false , 'error' : 'Missing Parameter: User'}))
			}
		});
		...
		ee_app.get('/api/loggedIn', function(req, res){
			res.contentType('application/json');
			res.end(JSON.stringify({'loggedIn' : req.LoginSession.authenticated, 'user' : req.LoginSession.user}));
		});
			...
			try {
				fs.readFile('./changelog.txt', function(err, data){
					if(err) { res.end(JSON.stringify({ success : false, error : err})); }
						res.end(JSON.stringify({success : true , result : JSON.parse(data.toString()) }))
				});
			} catch(e){
		...
		ee_app.get('/api/account', function(req, res){
			...
			if(req.LoginSession.authenticated == true){
				return ee_getAccountData(req.LoginSession.user, function(err, result){
					if(err){
						return res.end(JSON.stringify({'success' : false, 'error' : err}));
					} else {
						return res.end(JSON.stringify({'success' : true, 'result' : result}));
					}
				});
			} else {
				return res.end(JSON.stringify({'success' : false, 'error' : 'Unauthenticated'}));
			}
		});
			...
			if(req.LoginSession.authenticated == true){
				if(containsKeys(dat, ['characterName','server'])){
					...
					return ee_accountAddCharacter(dat, function(err, result){
						if(err){
							return res.end(JSON.stringify({'success': false , 'error' : err.toString()}));
						} else {
							return res.end(JSON.stringify({'success' : true, 'result' : result}));
						}
					});
				} else {
					return res.end(JSON.stringify({'success' : false , 'error' : 'Missing parameters: characterName or server'}));
				}
			} else {
				return res.end(JSON.stringify({'success' : false, 'error' : 'Unauthenticated'}));		
			}
		...
		ee_app.get('/api/account/removeCharacter', function(req, res){
			...
			if(req.LoginSession.authenticated == true){
				if(containsKeys(dat, ['characterName'])){
					...
					return res.end(JSON.stringify({'success' : false , 'error' : 'Missing parameters: characterName'}));
				}
			} else {
				return res.end(JSON.stringify({'success' : false, 'error' : 'Unauthenticated'}));		
			}
			...
			if(req.LoginSession.authenticated == true){
				if(containsKeys(dat, ['listingID'])){
					...
					return ee_closeListing(dat, function(err, result){
						if(err) return res.end(JSON.stringify({'success' : false , 'error': err.toString(), 'data': result}));
						return res.end(JSON.stringify({'success' : true, 'result' : true}));
					});
				} else {
					return res.end(JSON.stringify({'success' : false, 'error':'Missing parameter: listingID'}));
				}
			} else {
				return res.end(JSON.stringify({'success' : false, 'error' : 'Unauthenticated'}));				
			}
		...
		ee_app.get('/api/account/editListing', function(req, res){
			...
			if(req.LoginSession.authenticated == true){
				if(containsKeys(dat, ['listingID', 'fields[]']) == true){
					...
					return res.end(JSON.stringify({success : false, error : 'Missing parameters: listingID, fields'}))
				}
			} else {
				return res.end(JSON.stringify({success : false, error : "Unauthenticated"}))
			}
		...
		ee_app.get('/api/admin/removeListing', function(req,res){
			res.contentType('application/json');
			return res.end(JSON.stringify({'success':false, 'error':"Not Implemented"}));
			var dat = (url.parse(req.url).query != null) ? querystring.parse(url.parse(req.url).query) : {};
			if(req.LoginSession.user != "Maldaris"){
				return res.end(JSON.stringify({'success':false, 'error': "Not Logged in as Administrator."}));
			} else {
				if(containsKeys(dat, ['listingID'])){
					...
					return res.end(JSON.stringify({'success':false, 'error':"missing listingID"}));
				}
			}
		});
	}
